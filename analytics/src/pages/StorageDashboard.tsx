...
import { MongoStatsPlaylistResult } from '../types'
...
const dbIdWatchUrl = (dbId: string): string => {
...
const StorageDashboard: React.FC = () => {
  ...
  const renderResultName = ({ _id, result: { provider, title } }: MongoStatsPlaylistResult): JSX.Element => (
    <span>
  ...
  return (
    <div className={classes.root}>
      <Toolbar>
        ...
        <Button
          variant="contained"
          onClick={(): void => showInvalidateDialog(!invalidateDialog)}>
          Invalidate
        </Button>
      </Toolbar>
              ...
              <Grid item md={6} sm={12} xs={12} className={classes.table}>
                <LoadingPlaceholder loading={store.loading}>
                  <ValuesTableVis
                    ...
                    renderValue={({ hit }): string => hit.toString()} />
                </LoadingPlaceholder>
              </Grid>
              <Grid item md={6} sm={12} xs={12} className={classes.table}>
                <LoadingPlaceholder loading={store.loading}>
                  <ValuesTableVis
                    ...
                    renderName={renderResultName}
                    renderValue={({ lastModifiedDate }): string => moment(lastModifiedDate).calendar(null, {
                      sameDay: 'HH:mm',
                      lastDay: '[yesterday]',
      ...
      <ReloadButton onClick={(): void => store.reload(true)} />
      <InvalidateDialog
        providers={store.providers}
        open={invalidateDialog}
        onInvalidate={(provider, resultId): void => store.invalidate(provider, resultId)}
        onClose={(): void => showInvalidateDialog(false)} />
    </div>
  )
}

export default observer(StorageDashboard)
